...
function getshop(tid,cid,name,price,input,inputs,multi,isfaka,value,desc,alert,shopimg,close){
	$('#display_selectclass').hide();
	$('#shoplist').hide(); 
	$('#shopinfo').show();
	$("#selected").html('填写商品信息 <select name="tid" id="tid"><option value="'+tid+'" cid="'+cid+'" price="'+price+'" inputname="'+input+'" inputsname="'+inputs+'" multi="'+multi+'" isfaka="'+isfaka+'" count="'+value+'" alert="'+alert+'" shopimg="'+shopimg+'" close="'+close+'">'+name+'</option></select><a class="btn btn-success btn-xs pull-right" href="./?cid='+cid+'">返回重选</a>');
...
function getxiaohongshuid(){
	var songurl=$("#inputvalue").val();
	if(songurl==''){layer.alert('请确保每项不能为空！');return false;}
	if(songurl.indexOf('xiaohongshu.com')<0){layer.alert('请输入正确的链接！');return false;}
	try{
		var songid = songurl.split('item/')[1].split('?')[0];
	}catch(e){
		layer.alert('请输入正确的链接！');return false;
			...
			success : function(data) {
				...
				if(data.code == 0){
					...
					if(data.pay_tenpay>0){
						paymsg+='<button class="layui-btn layui-btn-primary layui-btn-fluid" onclick="dopay(\'tenpay\',\''+data.trade_no+'\')" style="margin-top:10px;"><img width="20" src="assets/icon/tenpay.ico" class="paylogo">财付通</button><br/>';
					}
					if (data.pay_rmb>0) {
