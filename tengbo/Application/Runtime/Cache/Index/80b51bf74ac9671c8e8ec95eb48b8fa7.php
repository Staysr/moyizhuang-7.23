...
<div class="container">
    <p class="container_nav"><a href="<?php echo U('Index/index');?>">首页</a> &nbsp>&nbsp <a href="" class="active">解决方案</a></p>
    <div class="theatre">
        <ul>
            <?php if(is_array($list1)): $i = 0; $__LIST__ = $list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['id'] == $_GET['id']): ?><li class="active  theatreli"><span onclick="foun(<?php echo ($vo['id']); ?>)"><?php echo ($vo["title"]); ?></span></li>
                <?php else: ?> <li class="theatreli"><span onclick="foun(<?php echo ($vo['id']); ?>)"><?php echo ($vo["title"]); ?></span></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    </div>
    <p class="the_title" id="title"><?php echo ($list["title"]); ?></p>
    <div class="theatre_container" id="content">
...
<script>
    function foun(id) {
        $.ajax({
            type:"post",
            dataType: 'json',
            url:"<?php echo U('Behavior/ajaxlist');?>",

            data:{id:id},
            success:function(res){
                console.log(res)
                //$("#content").remove();
                $("#title").html("<p>" + res.data.title +" </p>");
