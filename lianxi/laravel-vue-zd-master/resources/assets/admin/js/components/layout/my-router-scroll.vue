    ...
    <div class="layout-router">
        <div class="layout-router-scroll">
            ...
            <div class="layout-router-scroll-body">
                <Tag type="dot" v-for="(item, index) in openPageList" :key="index" :color="currentPage === item.name
                ? 'primary':'default'" :closable="item.name !== 'home'" :name="index" @on-close="tagClose"
                     @click.native="menuRouter(item.name)">{{ item.meta.title }}</Tag>
            </div>
        </div>
    </div>
</template>
