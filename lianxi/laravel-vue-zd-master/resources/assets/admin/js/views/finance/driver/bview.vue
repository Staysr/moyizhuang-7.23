    ...
    <component-modal title="账户明细" :width="70" >
        <my-lists v-model="lists.data" :columns="columns" @change="search" :loading="loading" >
            <Card>
                ...
                <alert>配送完成 <b>{{total[0]}}</b> 次，司机总价格  <b>{{total[1]}}</b> 元 </alert>
            </Card>
        </my-lists>
    </component-modal>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            search(page = 1){
                this.loading = true
                this.$http.get(`account/driver/`+this.data.driver_id, {params: this.request(page)}).then((res) => {
                    this.assignmentData(res.data.data)
                }).finally(() => {
                    this.loading = false
                });

                this.$http.get(`account/driver/statistics/`+this.data.driver_id, {params: this.request(page)}).then((res) => {
