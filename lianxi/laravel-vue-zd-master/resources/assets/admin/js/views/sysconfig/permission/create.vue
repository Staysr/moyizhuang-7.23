    ...
    <component-modal title="创建权限" :loading="loading">
        <Form ref="formCreate" :model="formCreate" :label-width="80">
            <FormItem label="上级菜单" prop="parent_id">
                <group-cascader v-model="formCreate.parent_id" placeholder="若为顶级菜单可不选择" :data="parents"></group-cascader>
            </FormItem>
            <FormItem label="菜单名称" prop="title" :rules="{required: true, message: '菜单名称不能为空'}">
                <Input v-model="formCreate.title" placeholder="菜单名称"></Input>
            </FormItem>
            <FormItem label="菜单路径" prop="name" :rules="{required: true, message: '菜单路径不能为空，且格式为 a-z.a-z ！', pattern: /[a-z]+\.[a-z]+/}">
                <Input v-model="formCreate.name" placeholder="菜单路径"></Input>
            </FormItem>
        ...
        <div slot="footer">
            <Button type="primary" :loading="loading" @click="createSubmit('formCreate', 'permission')">创建</Button>
        </div>
    </component-modal>
</template>
