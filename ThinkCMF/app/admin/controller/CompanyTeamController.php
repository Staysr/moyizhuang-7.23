 ...
 * )
 */
class CompanyTeamController extends AdminBaseController
{
    ...
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $data_insert['companyteam_time'] = time();


            $list = Db::name('companyteam')->insert($data_insert);
   ...
   public function menu_list(){
       $list = Db::name("companyteam")->select();
    ...
    {
        $id = input("id");
        $list = Db::name("companyteam")->where("id = $id")->delete();
        if ($list){
            $this->success("删除成功！");
        }else{
            $this->error("删除失败！");
