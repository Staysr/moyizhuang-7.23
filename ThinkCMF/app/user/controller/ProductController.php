...
class ProductController extends UserBaseController
{
    ...
    public function index()
    {
        ...
        $list = Db::name('product')->where($where)->field('post_title,thumbnail,post_keywords,id')->paginate(6);
    ...
    public function productinfo($id)
    {
        ...
        $list1 = Db::name('product')->where("id = $id")->field('post_code')->find();
        $comma_separated = implode("", $list1);
        $data = htmlspecialchars($comma_separated);
        $list2 = Db::name('asset')->where("post_code =$data")->field('file_path')->select();
        //print_r($list2);
        $this->assign("list", $list);
