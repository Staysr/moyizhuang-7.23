    ...
    if (isset($_GET['share'])) {
        if (isset($_GET['delshare']) && $_GET['delshare'] == 1 && isset($_GET['do']) && $_GET['do'] == 'del') {
            ...
            if ($id) {
                if ($ret = C::t('shares')->update_by_id($id, $share)) {
                    showTips(array('success' => true, 'shareurl' => outputurl($_G['siteurl'] . 'index.php?mod=shares&sid=' . dzzencode($ret)), 'shareid' => $ret));
                } else {
                ...
                if ($ret['success']) {
                    showTips(array('success' => true, 'shareurl' => outputurl($_G['siteurl'] . 'index.php?mod=shares&sid=' . dzzencode($ret['success'])), 'shareid' => $ret['success']));
                } else {
                    showTips(array('error' => lang('create_share_failer') . '！'), 'json');
                }
            }
        ...
        if ($shareid) {
            if ($share = C::t('shares')->fetch($shareid)) {
                $share['shareurl'] = outputurl($_G['siteurl'] . 'index.php?mod=shares&sid=' . dzzencode($share['id']));
