...
function xml_serialize($arr, $htmlon = FALSE, $isnormal = FALSE, $level = 1, $roottag = 'root') {
	...
	foreach($arr as $k => $v) {
		...
		{
			if(!is_array($v)) {
				$s .= $space."<$k>".($htmlon ? '<![CDATA[' : '').$v.($htmlon ? ']]>' : '')."</$k>\r\n";
			} else {
				$s .= $space."<$k>\r\n".xml_serialize($v, $htmlon, $isnormal, $level + 1).$space."</$k>\r\n";
			}			
		}
	}
	$s = preg_replace("/([\x01-\x08\x0b-\x0c\x0e-\x1f])+/", ' ', $s);
	return $level == 1 ? $s."</$roottag>" : $s;
}
