...
<?php 
$sql="Select * from SL_bsort where S_hide=0 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
$S_sh=$row["S_sh"];
...
if($S_sh==1){
$sh_info="发帖限制：需要审核";
$sql2="Select count(*) as B_count from SL_bbs where B_sort=".$row["S_id"]." and B_sh=1 and B_sub=0";
}else{
$sh_info="发帖限制：无需审核";
$sql2="Select count(*) as B_count from SL_bbs where B_sort=".$row["S_id"]." and B_sub=0";
}
if($S_lv==0){
$lv_info="浏览权限：游客";
}else{
...
if($S_sh==0){
$sql2="Select count(*) as B_count from SL_bbs where B_sort=".$row["S_id"]." and year(B_time)=".date("Y",strtotime(date('Y-m-d H:i:s')))." and month(B_time)=".date("m",strtotime(date('Y-m-d H:i:s')))." and day(B_time)=".date("d",strtotime(date('Y-m-d H:i:s')))." and B_sub=0";
}else{
$sql2="Select count(*) as B_count from SL_bbs where B_sort=".$row["S_id"]." and year(B_time)=".date("Y",strtotime(date('Y-m-d H:i:s')))." and month(B_time)=".date("m",strtotime(date('Y-m-d H:i:s')))." and day(B_time)=".date("d",strtotime(date('Y-m-d H:i:s')))." and B_sh=1 and B_sub=0";
}
