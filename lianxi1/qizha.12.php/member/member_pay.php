...
if($action=="submit"){
  ...
  for($i=0;$i<count($O_idsx);$i++){
    mysqli_query($conn, "update SL_orders set O_postage=".$_POST["postage"].",O_remark='".$_POST["remark"]."',O_no='".$gkey."' where O_id=".$O_idsx[$i]);
  }
  die();
}
...
if ($action == "bankpay") {
    if ($pay_type == "") {
        box("请选择付款方式!", "back", "error");
    } else {
        Header("Location: ../bank/alipayapi.php?O_id=" . $O_ids . "&WIDdefaultbank=" . $pay_type);
    }
}
...
for ($i = 0; $i < count($O_id); $i++) {
    ...
    $buy_list = $buy_list . "<table style='line-height:180%;width:100%'><tr><td width='15%'><img src='../" . splitx(splitx($P_path, "|", 0) , "__", 0) . "' height='80' style='border:#FFFFFF solid 5px; margin-right:10px;'></td><td width='40%'>商品名称：" . lang($P_title) . "<br><span style='color:#999999'>属性：" . $O_shuxing . "</span></td><td width='20%'>数量：" . $O_num . "</td><td width='25%'><span style='text-decoration:line-through;color:#999999'>价格：" . round($O_price * $O_num, 2) . "元</span><br><span style='color:#ff0000;font-weight:bold;'>折扣价：" . round($O_price * $O_num * $L_discount * 0.01, 2) . "元</span></td></tr></table>";
    $shuxing = "";
}
...
if ($_REQUEST["type"] == "balance") {
    ...
    if ($M_money - $money >= 0) {
        ...
        if ($x > 0) {
            mysqli_query($conn, "update SL_member set M_fen=M_fen+" . $money * $C_1yuan . " where M_id=" . $M_id);
            mysqli_query($conn, "update SL_member set M_money=M_money-" . $money . " where M_id=" . $M_id);
            mysqli_query($conn, "insert into SL_list(L_mid,L_title,L_change,L_time,L_no,L_type) values(" . $M_id . ",'购买商品',-" . $money . ",'" . date('Y-m-d H:i:s') . "','" . $P_code . "',0)");
            mysqli_query($conn, "insert into SL_list(L_title,L_mid,L_change,L_time,L_type,L_no) values('购买商品'," . $M_id . "," . $money * $C_1yuan . ",'" . date('Y-m-d H:i:s') . "',1,'" . $P_code . "')");
            sendmail("有订单已付款，请尽快发货", "<h2>您的网站“" . $C_webtitle2 . "”有订单已付款，请尽快发货</h2><hr>商品名称：" . $P_title . "<br>价格：" . $money . "元<br>付款方式：余额支付（" . $P_code . "）<br>状态：已付款（等待发货）<hr>请进入“网站后台” - “商城管理” - “订单管理”进行发货操作！", $C_email);
            box("支付成功，请等待发货", "index.php", "success");
        } else {
            box("订单已支付过！请勿重复操作", "member_order.php", "error");
        }
    } else {
        box("余额不足，请先充值！", "member_pay2.php?money=" . round($money - $M_money, 2) , "error");
    }
}
...
<div id="v">
<?php echo $buy_list?>
<input type='text' id='O_remark1' placeholder='留言/备注' class='form-control' style='width:100%;max-width:500px;'>
<div class="col-sm-4">
...
<div id="t">
<?php echo $buy_list?>
<input type='text' id='O_remark2' placeholder='留言/备注' class='form-control' style='width:100%;max-width:500px;'>
<div class="col-sm-4">
...
<div id="p">
<?php echo $buy_list?>
<input type='text' id='O_remark3' placeholder='留言/备注' class='form-control' style='width:100%;max-width:500px;'>
<div class="col-sm-4">
<form name="paypal" id="paypal" target="_blank" action="https://www.paypal.com/cgi-bin/webscr" method="post">    
<input type="hidden" name="cmd" value="_xclick"/>
 ...
 <input type="hidden" name="amount" value="<?php echo round($total_money/6.9,2)?>"/>
...
<li class="tab_con">
<?php echo $buy_list?>
<input type='text' id='O_remark4' placeholder='留言/备注' class='form-control' style='width:100%;max-width:500px;'>
<div class="col-sm-4">
...
function showcode(){
    $.ajax({
        ...
        data: "<?php echo "body = ".$P_title1." & attach = ".$O_ids." & total_fee = ".round($money,2)?>",
...
function banlance() {
    $.ajax({
        ...
        success: function(data) {
            document.location.href = '?O_id=<?php echo $O_ids?>&type=balance';
        }
    })
}
	</script>
