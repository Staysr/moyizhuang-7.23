...
$M_id=$_SESSION["M_id"];
if($M_id==""){
$M_id=getrs("select * from SL_member where M_login like '未提供'","M_id");
}
if($M_id==getrs("select * from SL_member where M_login like '未提供'","M_id")){
$url_to="index.php";
}else{
...
$Q_code=$_POST["Q_code"];
	if ($_POST["code"]!=$_SESSION["CmsCode"]){
		...
		$row = mysqli_fetch_assoc($result);
		if (mysqli_num_rows($result) > 0) {
		mysqli_query($conn,"update SL_query set Q_times=Q_times+1 where Q_code like '".$Q_code."'");
...
while($row2 = mysqli_fetch_assoc($result2)) {
			...
			mysqli_query($conn,"update SL_query set Q_first='".date('Y-m-d H:i:s')."' where Q_code like '".$Q_code."'");
