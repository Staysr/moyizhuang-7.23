...
if (strpos($_POST["body"], "|") !== false) {
    ...
    $M_id = getrs("select * from SL_orders where O_id=" . splitx($O_ids, ",", 0) , "O_member");
}
if ($_POST["trade_status"] = "TRADE_SUCCESS") {
    if (strpos($_POST["body"], "|") !== false) {
        $sql = "Select * from SL_list Where L_no like '" . $trade_no . "'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        if (mysqli_num_rows($result) <= 0) {
            mysqli_query($conn, "update SL_member set M_money=M_money+" . $body[1] . " where M_id=" . $body[0]);
        ...
        if (mysqli_num_rows($result) <= 0) {
            ...
            for ($i = 0; $i < count($O_id); $i++) {
                mysqli_query($conn, "update SL_orders set O_state=1,O_tradeno='" . $trade_no . "（支付宝付款）' where O_id=" . $O_id[$i]);
            }
            ...
            if ($C_1yuan2 > 0) {
                mysqli_query($conn, "insert into SL_list(L_title,L_mid,L_change,L_time,L_type,L_no) values('好友购买商品'," . getrs("select * from SL_member where M_id=" . $M_id, "M_from") . "," . $total_fee * $C_1yuan2 . ",'" . date('Y-m-d H:i:s') . "',1,'" . $trade_no . "')");
            }
