...
<nav class="navbar navbar-inverse navbar-fixed-top s_top" role="navigation">
    ...
    ?>
    <a href="member"><?php echo $_SESSION["M_login"]?></a> <a href="membe_login.php?action=unlogin"><?php echo lang("退出/l/Sign Out")?></a>
    <?php }?>
    </div>
    </div>
    </div>
...
$sql="select * from SL_menu where U_sub=0 && U_hide=0 order by U_order,U_id desc";
$result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
...
$sql="select * from SL_news,SL_nsort where (N_title like '%".$keyword."%' or N_content like '%".$keyword."%' ) and N_sort=S_id order by N_id desc";
$result = mysqli_query($conn, $sql);
...
$sql="select * from SL_product,SL_psort where (P_title like '%".$keyword."%' or P_content like '%".$keyword."%' ) and P_sort=S_id order by P_id desc";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
