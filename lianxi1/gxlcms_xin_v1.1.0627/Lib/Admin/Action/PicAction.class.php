...
class PicAction extends BaseAction
{
	public function down()
	{
		...
		if ("fail" == trim($_GET["id"])) {
			$rs->execute("update " . C("db_prefix") . "ting set ting_pic=REPLACE(ting_pic,\"https://\", \"http://\")");
		}

		$count = $rs->where("Left(ting_pic,7)=\"http://\"")->count("ting_id");
		if(!$count){
			$count = $rs->where("Left(ting_pic,8)=\"https://\"")->count("ting_id");
		}
		$list = $rs->where("Left(ting_pic,7)=\"http://\"")->order("ting_addtime desc")->limit(C("upload_http_down"))->select();
		if(!$list){
			
			$list = $rs->where("Left(ting_pic,8)=\"https://\"")->order("ting_addtime desc")->limit(C("upload_http_down"))->select();
		...
		if ($list) {
			...
			foreach ($list as $key => $value ) {
				...
				if ($value["ting_pic"] == $imgnew) {
					$rs->where("ting_id=" . $value["ting_id"])->setField("ting_pic", str_replace("http://", "https://", $value["ting_pic"]));
