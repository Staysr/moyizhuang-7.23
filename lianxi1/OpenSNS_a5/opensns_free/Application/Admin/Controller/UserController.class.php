...
//        $map['status'] = array('egt', 0);
    ...
    public function changeStatus($method = null)
    {
        ...
        switch (strtolower($method)) {
            case 'forbiduser':
                ...
                foreach($id as $k){
                    $session=M('member')->where(array('uid'=>$k))->getField('session_id');
                    $res1=D('session')->where(array('session_id'=>$session))->delete();
                    if($res1 === false){
                        $this->error("删除用户session失败~") ;
                    }
                }
    ...
    public function ForbidUser() {
        ...
        if(IS_POST){
            ...
            foreach ($id as &$val){
                $session_id = D('Member')->field('session_id')->find($val);
                M('session')->where(array('session_id'=>$session_id['session_id']))->setField(array('session_data'=>''));
            }
            unset($val);
