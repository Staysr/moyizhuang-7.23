...
class RoleController extends AdminController
{
    ...
    public function userList($page = 1, $r = 20)
    {
        ...
        $script_url = './Application/admin/Static/js/inituser.js' ;
        $css_url =  './Application/admin/Static/css/initloading.css' ;
        $builder->buttonModalPopup(U('Role/changeRole', array('role_id' => $map_user_list['role_id'])), array(), L('_MIGRATING_USER_'), array('data-title' => L('_MIGRATING_USER_TO_ANOTHER_IDENTITY_'), 'target-form' => 'ids'))
            ->button(L('_INITIALIZE_THE_USER_'), array('href' => 'javascript:void(0);', 'data-role'=>'doInitUser', 'data-url'=>U('role/initunhaveuser')))
...
//                    $this->error(L('_PLEASE_UPLOAD_YOUR_AVATAR_'));
    ...
    {
        $type = I('type', 0, 'intval') ;//强制重新获取数据
        $memberModel = D('Common/Member');
        $init_user_list = unserialize(S('unhave_user_list')) ;
        $return = array() ;
        $return['status'] = -1 ;
        $lock_file = "./Conf/initUnhaveList.lock" ;
        if(file_exists($lock_file) && $init_user_list['time']+30<=time()){
            $return['info'] = "正在初始化中~" ;
            $this->success($return['info'],U('userList'),2);
        }
        $lock_files = @fopen($lock_file, 'a+') ;
        if($lock_files == false) {
            $return['status'] = 0 ;
            $return['info'] = "读写锁失败，请检查读写权限~" ;
            $this->success($return['info'],U('userList'),2);
        }
        @fwrite($lock_files, '初始化没有身份的用户锁~');
        fclose($lock_files) ;
        if($init_user_list['list'] == false || $type == 1){
            $uids = $memberModel->field('uid')->select();
            $not_have_uids = $uids = array_column($uids, 'uid');
            ...
            $number = modC('INIT_USER_NUMBER', 500, 'USERCONFIG') ;
            $listAll = array_chunk($not_have_uids, $number) ;
            $return['step_number'] = count($listAll) ;
            $now_init_list = array_shift($listAll) ;
        }else{
            $now_init_list = array_shift($init_user_list['list']) ;
            $listAll = $init_user_list['list'] ;
        }
        ...
        unset($val);
        $res = $this->userRoleModel->addAll($dataList);
        $file_unlick = @unlink($lock_file) ;
        if($file_unlick == false && file_exists($lock_file)) {
            $return['status'] = 0 ;
            $return['info'] = "删除文件锁失败，请检查读写权限~" ;
            $this->success($return['info'],U('userList'),2);
        }
        $session_unlock = array(
            'list'   => $listAll ,
            'time'   => time()
        ) ;
        $session_unlock = serialize($session_unlock) ;
        S('unhave_user_list',$session_unlock,60*10) ;
        if($res){
            if(empty($listAll)) {
                $return['status'] = 1 ;
                $return['info'] = '初始化完成~' ;
