...
//dump($messages);exit;
    ...
    public function postMessage($content, $talk_id)
    {
        ...
        $uids[1]=str_replace("[","",$uids[1]);
        $uids[1]=str_replace("]","",$uids[1]);
        $uids[0]=str_replace("[","",$uids[0]);
        $uids[0]=str_replace("]","",$uids[0]);

        $from_uid=query_user(array('avatar128','uid','nickname','space_url','rank_link','space_link'),is_login());
        if($uids[1]!=is_login()){
            send_message($uids[1],'您收到了一条新消息，快去看看吧。',$from_uid['nickname'].'给您发了一条消息,快去看看是什么吧！');
        }
        if($uids[0]!=is_login()){
            send_message($uids[0],'您收到了一条新消息，快去看看吧。',$from_uid['nickname'].'给您发了一条消息,快去看看是什么吧！');
        }
