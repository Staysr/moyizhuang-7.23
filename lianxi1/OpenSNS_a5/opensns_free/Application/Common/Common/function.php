...
function action_log($action = null, $model = null, $record_id = null, $user_id = null)
{
    ...
    if (!empty($action_info['rule'])) {
        ...
        //执行行为
        $res = execute_action($rules, $action_info['id'], $user_id, $log_id, $log_score,$action);
    } else {
        if ($log_score) {
 ...
 * @author 郑钟良<zzl@ourstu.com>
 */
function execute_action($rules = false, $action_id = null, $user_id = null, $log_id = null, $log_score = '',$action='')
{
    ...
    $select_log = M('ActionLog')->where(array('user_id'=>$user_id,'action_id'=>$action_log['action_id'],'is_score'=>1))->select();
    foreach ($rules as $rule) {
        //检查执行周期
        $map = array('action_id' => $action_id, 'user_id' => $user_id);
//        $map['create_time'] = array('gt', NOW_TIME - intval($rule['cycle']) * 3600);
//        $exec_count = M('ActionLog')->where($map)->count();
        $map['create_time'] = NOW_TIME - intval($rule['cycle']) * 3600;
        if ($map['create_time'] < $select_log[count($select_log)-$rule['max']]['create_time']) {
            continue;
        }
        //执行数据库操作
        ...
        if($action=='reg'){
            $res = $Model->where(array('uid' => $user_id, 'status' => 1))->setField($field, array('exp', $field . $rule['rule']));
        }
...
//        $action = strpos($rule['rule'], '-') ? 'dec' : 'inc';
    ...
    if ($log_score) {
        ...
        $map1['is_score'] = 1;
        M('ActionLog')->where(array('id' => $log_id))->setField('remark', array('exp', "CONCAT(remark,'" . $log_score . "')"));
        M('ActionLog')->where(array('id'=>$log_id))->save($map1);
    }
    return $return;
}
