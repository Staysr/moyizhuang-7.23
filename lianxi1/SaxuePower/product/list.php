<?php
if ( $saxueMenu[SAXUE_LANGUAGE][$_REQUEST['catid']]['child'] && $saxueColumn[$_REQUEST['catid']]['display'] && ( file_exists( SAXUE_THEME_PATH . '/product/' . $temp_setting['column_template'] ) || file_exists( SAXUE_THEME_PATH . '/product/column.html' ) ) ) {
		$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/product/' . $temp_setting['column_template'];
		if ( !file_exists( $saxueTset['saxue_page_template'] ) ) {
				$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/product/column.html';
		}
		...
		$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/product/' . $temp_setting['list_template'];
		if ( !file_exists( $saxueTset['saxue_page_template'] ) ) {
				$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/product/list.html';
		}
		...
		if ( !$page_used_cache ) {
				...
				if ( SAXUE_USE_CACHE ) {
						saxue_getcachevars( "productlistlog" );
						if ( !is_array( $saxueProductlistlog ) ) {
								$saxueProductlistlog = array();
						} 
						if ( !isset( $saxueProductlistlog[$pagecacheid] ) || SAXUE_CACHE_LIFETIME < SAXUE_NOW_TIME - $saxueProductlistlog[$pagecacheid]['time'] ) {
								$saxueProductlistlog[$pagecacheid] = array( "rows" => $data_handler -> getcount( $criteria ), "time" => SAXUE_NOW_TIME );
								saxue_setcachevars( "productlistlog", $saxueProductlistlog );
						} 
						$listrows = $saxueProductlistlog[$pagecacheid]['rows'];
				} else {
