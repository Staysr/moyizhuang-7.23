...
if ( !empty( $_REQUEST['batchaction'] ) && !empty( $_REQUEST['ids'] ) && is_array( $_REQUEST['ids'] ) && count( $_REQUEST['ids'] ) > 0 ) {
		switch ( $_REQUEST['batchaction'] ) {
				case 'delete':
						$data_handler -> db -> query( "DELETE FROM " . saxue_dbprefix( "job" ) . " WHERE id IN(" . implode( ',', $_REQUEST['ids'] ) . ")" );
						break;
				case 'nodisplay':
...
$saxueTset['saxue_contents_template'] = SAXUE_WEB_PATH . "/job/templates/index.html";
