<?php
if ( $saxueMenu[SAXUE_LANGUAGE][$_REQUEST['catid']]['child'] && $saxueColumn[$_REQUEST['catid']]['display'] && ( file_exists( SAXUE_THEME_PATH . '/article/' . $temp_setting['column_template'] ) || file_exists( SAXUE_THEME_PATH . '/article/column.html' ) ) ) {
		$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/article/' . $temp_setting['column_template'];
		if ( !file_exists( $saxueTset['saxue_page_template'] ) ) {
				$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/article/column.html';
		}
		...
		$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/article/' . $temp_setting['list_template'];
		if ( !file_exists( $saxueTset['saxue_page_template'] ) ) {
				$saxueTset['saxue_page_template'] = SAXUE_THEME_PATH . '/article/list.html';
		}
		...
		if ( !$page_used_cache ) {
				...
				if ( SAXUE_USE_CACHE ) {
						saxue_getcachevars( "articlelistlog" );
						if ( !is_array( $saxueArticlelistlog ) ) {
								$saxueArticlelistlog = array();
						} 
						if ( !isset( $saxueArticlelistlog[$pagecacheid] ) || SAXUE_CACHE_LIFETIME < SAXUE_NOW_TIME - $saxueArticlelistlog[$pagecacheid]['time'] ) {
								$saxueArticlelistlog[$pagecacheid] = array( "rows" => $data_handler -> getcount( $criteria ), "time" => SAXUE_NOW_TIME );
								saxue_setcachevars( "articlelistlog", $saxueArticlelistlog );
						} 
						$listrows = $saxueArticlelistlog[$pagecacheid]['rows'];
				} else {
